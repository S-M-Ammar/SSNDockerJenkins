...
let mac_bytes_dict_mqtt = {}

let update_node_mac_dict = async(msg)=>{

    return new Promise(((resolve, reject) => {

        try{
            let params = decipher_message(msg)
            mac_bytes_dict_mqtt[params.node_id] = [msg[0],msg[1],msg[2],msg[3],msg[4],msg[5]]
            resolve("Node mac bytes recorded for mqtt")
        }
        catch (e) {
            reject(e)
        }
...
let send_time_of_day_message = async (msg,client)=>{
    return new Promise(((resolve, reject) => {
        ...
        try {
            ...
            let mac_id_string = utils.get_MAC_id_from_bytes(msg[0],msg[1],msg[2],msg[3],msg[4],msg[5])
            client.publish(mac_id_string.toUpperCase(),message,{qos:1})
...
let communication_mqtt = async (topic,msg,client)=>{


    console.log(await update_node_mac_dict(msg))

    if(msg[6]==3){
        return await send_time_of_day_message(msg,client)
    }
    if(msg[6]==5){
...
module.exports = {communication_mqtt,mac_bytes_dict_mqtt}